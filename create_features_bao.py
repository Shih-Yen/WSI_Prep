...
class WSIDataset(Dataset):
    ...
    def has_target_mag(self):   # check if the wsi can be downsampled to obtain the targeted mag
        ...
        if self.args.target_mag in native_magnifications:
            return native_magnifications[self.args.target_mag]
        else:
            None
...
def process_tiles_pt(args, slide_id: str, models: Dict[str, nn.Module], progress=0):
    ...
    if wsi_dataset.has_target_mag() is None:
        raise ValueError(f"No magnification {args.target_mag} found for slide {slide_id}")
