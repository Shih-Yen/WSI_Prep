...
def process_tiles_pt_singleMdl(args, slide_id: str, models: Dict[str, nn.Module], progress=0):
    '''
    process the tiles for a single model
    Input:
        args: the arguments
        slide_id: str, the slide id
        models: Dict[str, nn.Module], the model to extract features (only one model)
        progress: float, the progress of the processing
    '''
    assert len(models) == 1, "only one model is allowed in this function"
    ...
    if args.stain_norm:
        ...
        stain_norm_str = ''
    feat_h5_dir = f"{args.feat_folder}/{model_name}/{args.target_mag}X/h5_files{stain_norm_str}"
    feat_pt_dir = f"{args.feat_folder}/{model_name}/{args.target_mag}X/pt_files{stain_norm_str}"
    os.makedirs(feat_h5_dir, exist_ok=True)
    os.makedirs(feat_pt_dir, exist_ok=True)
    ...
    for count, (batch, coords) in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            feat_name = list(out.keys())[0] ## assuming only one model is used
...
def main():
    ...
    for idx, slide_id in enumerate(slide_ids):
        try:
            for mdl_key, mdl in models.items():
                ## In this function, only one model is allowed per call
                process_tiles_pt_singleMdl(args, slide_id, {mdl_key: mdl}, progress=idx/len(slide_ids))
